  ...
  PUBLIC SECTION.
    ...
    CLASS-METHODS split_path
      IMPORTING
        iv_path             TYPE string
      RETURNING
        VALUE(rv_path_name) TYPE zif_mcp_ajson_types=>ty_path_name.
    ...
    CLASS-METHODS escape_key
      IMPORTING
        iv_key                TYPE string
      RETURNING
        VALUE(rv_escaped_key) TYPE string.

    CLASS-METHODS unescape_key
      IMPORTING
        iv_escaped_key TYPE string
      RETURNING
        VALUE(rv_key)  TYPE string.

    CLASS-METHODS split_path_with_escaping
      IMPORTING
        iv_path             TYPE string
      RETURNING
        VALUE(rs_path_name) TYPE zif_mcp_ajson_types=>ty_path_name.

    CLASS-METHODS normalize_path_with_escaping
      IMPORTING
        iv_path                   TYPE string
      RETURNING
        VALUE(rv_normalized_path) TYPE string.

    CLASS-METHODS split_path_into_segments
      IMPORTING
        iv_path            TYPE string
      RETURNING
...
CLASS lcl_utils IMPLEMENTATION.
  ...
  METHOD validate_array_index.
    ...
    IF NOT iv_index CO '0123456789'.
      zcx_mcp_ajson_error=>raise( |Cannot add non-numeric key [{ iv_index }] to array [{ iv_path }]| ) ##NO_TEXT.
    ENDIF.
    rv_index = iv_index.
    IF rv_index = 0.
      zcx_mcp_ajson_error=>raise( |Cannot add zero key to array [{ iv_path }]| ) ##NO_TEXT.
    ENDIF.
    ...
    CASE lo_type->type_kind.
      ...
      WHEN lif_kind=>table.
        ...
        IF lo_table_type->table_kind <> cl_abap_tabledescr=>tablekind_std.
          zcx_mcp_ajson_error=>raise( 'Unsupported type of input table (must be standard table)' ) ##NO_TEXT.
        ENDIF.
        TRY.
          ...
          CATCH cx_root.
            zcx_mcp_ajson_error=>raise( 'Error converting input table (should be string_table)' ) ##NO_TEXT.
        ENDTRY.
      WHEN OTHERS.
        zcx_mcp_ajson_error=>raise( 'Unsupported type of input (must be char, string, string_table, or xstring)' ) ##NO_TEXT.
    ENDCASE.
  ...
  METHOD escape_key.
    " Use JSON Pointer escaping: ~ becomes ~0, / becomes ~1
    " Order matters: escape ~ first, then /
    rv_escaped_key = replace(
      val = iv_key
      sub = '~'
      with = '~0'
      occ = 0 ).
  ...
  METHOD unescape_key.
    " Use JSON Pointer unescaping: ~1 becomes /, ~0 becomes ~
  ...
  METHOD split_path_with_escaping.
    DATA lt_segments TYPE string_table.
    DATA lv_current_segment TYPE string.
    DATA lv_i TYPE i.
    DATA lv_len TYPE i.

    lv_len = strlen( iv_path ).

    " Handle root path cases same as original split_path
    IF lv_len = 0 OR iv_path = '/'.
      RETURN. " empty path is the alias for root item = '' + ''
    ENDIF.

    " Parse character by character to handle JSON Pointer escaping
    lv_i = 0.
    WHILE lv_i < lv_len.
      DATA lv_char TYPE c.
      lv_char = iv_path+lv_i(1).

      IF lv_char = '/'.
        " Path separator
        IF lv_current_segment IS NOT INITIAL.
          APPEND lv_current_segment TO lt_segments.
          CLEAR lv_current_segment.
        ENDIF.
        ADD 1 TO lv_i.
      ELSEIF lv_char = '~' AND lv_i + 1 < lv_len.
        " Potential escape sequence
        DATA lv_next_pos TYPE i.
        DATA lv_next_char TYPE c.
        lv_next_pos = lv_i + 1.
        lv_next_char = iv_path+lv_next_pos(1).

        IF lv_next_char = '0' OR lv_next_char = '1'.
          " Valid escape sequence - add both characters
          lv_current_segment = lv_current_segment && lv_char && lv_next_char.
          ADD 2 TO lv_i.
        ELSE.
          " Not an escape sequence, treat as regular character
          lv_current_segment = lv_current_segment && lv_char.
          ADD 1 TO lv_i.
        ENDIF.
      ELSE.
        " Regular character
    ...
    " Build result - same logic as original split_path
    DATA lv_segments TYPE i.
    lv_segments = lines( lt_segments ).

    IF lv_segments = 0.
      " This shouldn't happen due to root path check above, but keep for safety
      RETURN.
    ELSEIF lv_segments = 1.
      " Single level
      rs_path_name-path = '/'.
      READ TABLE lt_segments INDEX 1 INTO rs_path_name-name.
    ELSE.
      " Multi level - last segment is name, rest is path
      READ TABLE lt_segments INDEX lv_segments INTO rs_path_name-name.
      DELETE lt_segments INDEX lv_segments.
      rs_path_name-path = '/'.
      LOOP AT lt_segments INTO DATA(lv_segment).
        rs_path_name-path = rs_path_name-path && lv_segment && '/'.
      ENDLOOP.
    ENDIF.
  ENDMETHOD.

  METHOD normalize_path_with_escaping.
    " Handle root path same as original
    IF iv_path IS INITIAL OR iv_path = '/'.
      rv_normalized_path = '/'.
      RETURN.
    ENDIF.

    " For non-root paths, we need escaping-aware normalization
    " First, ensure it starts with /
    IF iv_path+0(1) <> '/'.
      rv_normalized_path = '/' && iv_path.
    ELSE.
      rv_normalized_path = iv_path.
    ENDIF.

    " Then ensure it ends with / (but be careful about escaped sequences)
    DATA lv_len TYPE i.
    DATA lv_last_char TYPE c.
    DATA lv_len_prev TYPE i.

    lv_len = strlen( rv_normalized_path ).
    IF lv_len > 0.
      lv_len_prev = lv_len - 1.
      lv_last_char = rv_normalized_path+lv_len_prev(1).

      " Add trailing / if the last character isn't /
      IF lv_last_char <> '/'.
        rv_normalized_path = rv_normalized_path && '/'.
      ENDIF.
    ENDIF.
  ENDMETHOD.

  METHOD split_path_into_segments.
    DATA lv_i TYPE i.
    DATA lv_len TYPE i.
    DATA lv_char TYPE c.
    DATA lv_current_segment TYPE string.
    ...
    " Parse path character by character to handle escaping
    WHILE lv_i < lv_len.
      lv_char = iv_path+lv_i(1).

      IF lv_char = '~' AND lv_i + 1 < lv_len.
        " Potential escape sequence
        DATA lv_next_pos TYPE i.
        DATA lv_next_char TYPE c.
        lv_next_pos = lv_i + 1.
        ...
        IF lv_next_char = '0' OR lv_next_char = '1'.
          " Valid escape sequence - add both characters to current segment
          lv_current_segment = lv_current_segment && lv_char && lv_next_char.
          ADD 2 TO lv_i.
        ELSE.
          " Not an escape sequence, treat as regular character
      ...
      ELSEIF lv_char = '/'.
        " Path separator
        IF lv_current_segment IS NOT INITIAL.
          APPEND lv_current_segment TO rt_segments.
          CLEAR lv_current_segment.
        ENDIF.
        ADD 1 TO lv_i.
      ELSE.
...
CLASS lcl_json_parser DEFINITION FINAL.
  PUBLIC SECTION.
    ...
    METHODS parse
      ...
      RETURNING
        VALUE(rt_json_tree) TYPE zif_mcp_ajson_types=>ty_nodes_tt
      RAISING
        zcx_mcp_ajson_error.
  ...
  PRIVATE SECTION.
    ...
    TYPES:
      ty_stack_tt TYPE STANDARD TABLE OF REF TO zif_mcp_ajson_types=>ty_node.
    ...
    METHODS _parse
      ...
      RETURNING
        VALUE(rt_json_tree) TYPE zif_mcp_ajson_types=>ty_nodes_tt
      RAISING
        zcx_mcp_ajson_error cx_dynamic_check. " cx_sxml_error is not released on Steampunk #153
            ...
            ELSE.
              ...
              LOOP AT lt_attributes INTO lo_attr.
                IF lo_attr->qname-name = 'name' AND lo_attr->value_type = if_sxml_value=>co_vt_text.
                  " Escape the key name when storing internally
                  <item>-name = lcl_utils=>escape_key( lo_attr->get_value( ) ).
                ENDIF.
              ENDLOOP.
...
CLASS lcl_json_serializer DEFINITION FINAL CREATE PRIVATE.
  PUBLIC SECTION.
    ...
    CLASS-METHODS stringify
      IMPORTING
        it_json_tree          TYPE zif_mcp_ajson_types=>ty_nodes_ts
  ...
  PRIVATE SECTION.
    ...
    DATA mt_json_tree TYPE zif_mcp_ajson_types=>ty_nodes_ts.
    DATA mv_keep_item_order TYPE abap_bool.
...
CLASS lcl_json_serializer IMPLEMENTATION.
  ...
  METHOD stringify_node.
    ...
    DATA lv_unescaped_name TYPE string.
    ...
    IF is_node-name IS NOT INITIAL AND is_node-index IS INITIAL. " Not root, not array item
      " Unescape the key name for JSON output
      lv_unescaped_name = lcl_utils=>unescape_key( is_node-name ).

      IF mv_indent_step > 0.
        lv_item = lv_item && |"{ lv_unescaped_name }": |.
      ELSE.
        lv_item = |"{ lv_unescaped_name }":|.
      ENDIF.
    ENDIF.
    ...
    CASE is_node-type.
      ...
      WHEN zif_mcp_ajson_types=>node_type-object.
        lv_item = lv_item && '{'.
      WHEN zif_mcp_ajson_types=>node_type-string.
        lv_item = lv_item && |"{ escape_string( is_node-value ) }"|.
      WHEN zif_mcp_ajson_types=>node_type-boolean OR zif_mcp_ajson_types=>node_type-number.
    ...
    IF mv_indent_step > 0
      AND ( is_node-type = zif_mcp_ajson_types=>node_type-array OR is_node-type = zif_mcp_ajson_types=>node_type-object )
...
CLASS lcl_json_to_abap DEFINITION FINAL.
  PUBLIC SECTION.
    ...
    METHODS constructor
      IMPORTING
        ...
        !ii_custom_mapping TYPE REF TO zif_mcp_ajson_mapping OPTIONAL.

    METHODS to_abap
      IMPORTING
        it_nodes    TYPE zif_mcp_ajson_types=>ty_nodes_ts
      CHANGING
    ...
    METHODS to_timestamp
      IMPORTING
        iv_value         TYPE zif_mcp_ajson_types=>ty_node-value
      RETURNING
  ...
  PRIVATE SECTION.
    ...
    DATA mr_nodes TYPE REF TO zif_mcp_ajson_types=>ty_nodes_ts.
    DATA mi_custom_mapping TYPE REF TO zif_mcp_ajson_mapping.
    ...
    METHODS get_node_type
      IMPORTING
        is_node             TYPE zif_mcp_ajson_types=>ty_node OPTIONAL " Empty for root
        is_parent_type      TYPE ty_type_cache OPTIONAL
...
CLASS lcl_json_to_abap IMPLEMENTATION.
  ...
  METHOD get_node_type.
    ...
    IF sy-subrc <> 0.
      ...
      CASE is_parent_type-type_kind.
        ...
        WHEN lif_kind=>struct_flat OR lif_kind=>struct_deep.
          ...
          IF sy-subrc <> 0.
            IF mv_corresponding = abap_false.
              zcx_mcp_ajson_error=>raise( |Path not found| ) ##NO_TEXT.
            ELSE.
              CLEAR rs_node_type.
              RETURN.
            ENDIF.
        ...
        WHEN OTHERS.
          zcx_mcp_ajson_error=>raise( |Unexpected parent type| ) ##NO_TEXT.
      ENDCASE.
  ...
  METHOD any_to_abap.
    ...
    DATA lx_ajson TYPE REF TO zcx_mcp_ajson_error.
    DATA lx_root TYPE REF TO cx_root.
    DATA lr_target_field TYPE REF TO data.

    FIELD-SYMBOLS <n> TYPE zif_mcp_ajson_types=>ty_node.
    ...
    TRY.
        ...
        LOOP AT mr_nodes->* ASSIGNING <n> USING KEY array_index WHERE path = iv_path.
          ...
          IF ls_node_type-type_kind = lif_kind=>data_ref OR
            ...
            zcx_mcp_ajson_error=>raise( 'Cannot assign to ref' ) ##NO_TEXT.
          ENDIF.
          ...
          CASE is_parent_type-type_kind.
            WHEN lif_kind=>table.
              IF NOT ls_node_type-target_field_name CO '0123456789'.
                ...
                zcx_mcp_ajson_error=>raise( 'Need index to access tables' ) ##NO_TEXT.
              ENDIF.
          ...
          CASE <n>-type.
            WHEN zif_mcp_ajson_types=>node_type-object.
              IF ls_node_type-type_kind <> lif_kind=>struct_flat AND
                ...
                zcx_mcp_ajson_error=>raise( 'Expected structure' ) ##NO_TEXT.
              ENDIF.
              any_to_abap(
            ...
            WHEN zif_mcp_ajson_types=>node_type-array.
              IF NOT ls_node_type-type_kind = lif_kind=>table.
                zcx_mcp_ajson_error=>raise( 'Expected table' ) ##NO_TEXT.
              ENDIF.
              any_to_abap(
                iv_path         = <n>-path && <n>-name && '/'
                is_parent_type  = ls_node_type
          ...
          IF is_parent_type-tab_item_buf IS BOUND. " Indirect hint that table was sorted/hashed, see get_node_type.
            TRY.
                ...
                IF sy-subrc <> 0.
                  zcx_mcp_ajson_error=>raise( 'Duplicate insertion' ) ##NO_TEXT.
                ENDIF.
      ...
      CATCH zcx_mcp_ajson_error INTO lx_ajson.
  ...
  METHOD value_to_abap.
    ...
    IF is_node_type-type_kind CA lif_kind=>deep_targets.
      zcx_mcp_ajson_error=>raise( |Unsupported target for value [{ is_node_type-type_kind }]| ) ##NO_TEXT.
    ENDIF.
    ...
    CASE is_node-type.
      ...
      WHEN zif_mcp_ajson_types=>node_type-number.
        " TODO: check type ?
      ...
      WHEN zif_mcp_ajson_types=>node_type-string.
        ...
        ENDIF.
      WHEN OTHERS.
        zcx_mcp_ajson_error=>raise( |Unexpected JSON type [{ is_node-type }]| ) ##NO_TEXT.
    ENDCASE.
  ...
  METHOD to_date.
    ...
    IF sy-subrc <> 0.
      zcx_mcp_ajson_error=>raise( 'Unexpected date format' ) ##NO_TEXT.
    ENDIF.
  ...
  METHOD to_timestamp.
    ...
    ELSE.
      ...
      IF sy-subrc <> 0.
        zcx_mcp_ajson_error=>raise( 'Unexpected timestamp format' ) ##NO_TEXT.
      ENDIF.
    ...
    TRY.
      ...
      CATCH cx_parameter_invalid_range cx_parameter_invalid_type.
        zcx_mcp_ajson_error=>raise( 'Unexpected error calculating timestamp' ) ##NO_TEXT.
    ENDTRY.
  ...
  METHOD to_time.
    ...
    IF sy-subrc <> 0.
      zcx_mcp_ajson_error=>raise( 'Unexpected time format' ) ##NO_TEXT.
    ENDIF.
...
CLASS lcl_abap_to_json DEFINITION FINAL.
  PUBLIC SECTION.
    ...
    CLASS-METHODS convert
      IMPORTING
        iv_data           TYPE any
        is_prefix         TYPE zif_mcp_ajson_types=>ty_path_name OPTIONAL
        iv_array_index    TYPE i DEFAULT 0
        ii_custom_mapping TYPE REF TO zif_mcp_ajson_mapping OPTIONAL
        is_opts           TYPE zif_mcp_ajson=>ty_opts OPTIONAL
        iv_item_order     TYPE i DEFAULT 0
      RETURNING
        VALUE(rt_nodes)   TYPE zif_mcp_ajson_types=>ty_nodes_tt
      RAISING
        zcx_mcp_ajson_error.
    ...
    CLASS-METHODS insert_with_type
      IMPORTING
        iv_data           TYPE any
        iv_type           TYPE zif_mcp_ajson_types=>ty_node_type
        is_prefix         TYPE zif_mcp_ajson_types=>ty_path_name OPTIONAL
        iv_array_index    TYPE i DEFAULT 0
        ii_custom_mapping TYPE REF TO zif_mcp_ajson_mapping OPTIONAL
        is_opts           TYPE zif_mcp_ajson=>ty_opts OPTIONAL
  ...
  PRIVATE SECTION.
    ...
    METHODS convert_any
      IMPORTING
        ...
        is_prefix     TYPE zif_mcp_ajson_types=>ty_path_name
        iv_index      TYPE i DEFAULT 0
        iv_item_order TYPE i DEFAULT 0
      CHANGING
        ct_nodes      TYPE zif_mcp_ajson_types=>ty_nodes_tt
      RAISING
        zcx_mcp_ajson_error.

    METHODS convert_ajson
      IMPORTING
        io_json       TYPE REF TO zif_mcp_ajson
        is_prefix     TYPE zif_mcp_ajson_types=>ty_path_name
        iv_index      TYPE i DEFAULT 0
        iv_item_order TYPE i DEFAULT 0
      CHANGING
...
CLASS lcl_abap_to_json IMPLEMENTATION.
  ...
  METHOD class_constructor.
    ...
    DATA lo_dummy TYPE REF TO zcl_mcp_ajson.
        ...
        ELSEIF io_type->type_kind = lif_kind=>object_ref
          ...
          convert_ajson(
            ...
            CHANGING
              ct_nodes = ct_nodes ).
        ELSE.
          zcx_mcp_ajson_error=>raise( |Unsupported type [{ io_type->type_kind
  ...
  METHOD convert_value.
    ...
    IF io_type->absolute_name = '\TYPE-POOL=ABAP\TYPE=ABAP_BOOL'
      ...
      ls_node-type = zif_mcp_ajson_types=>node_type-boolean.
    ...
    ELSEIF io_type->absolute_name = '\TYPE=TIMESTAMP'.
      IF mv_format_datetime = abap_true.
        ls_node-type  = zif_mcp_ajson_types=>node_type-string.
        ls_node-value = format_timestamp( iv_data ).
      ELSE.
        ls_node-type  = zif_mcp_ajson_types=>node_type-number.
    ...
    ELSEIF io_type->type_kind CO lif_kind=>numeric.
      ...
      ls_node-value = |{ iv_data }|.
    ELSE.
      zcx_mcp_ajson_error=>raise( |Unexpected elementary type [{
  ...
  METHOD convert_ref.
    ...
    IF iv_data IS INITIAL.
      ls_node-type  = zif_mcp_ajson_types=>node_type-null.
      ls_node-value = 'null'.
    ELSE.
      " TODO support data references
      zcx_mcp_ajson_error=>raise( |Unexpected reference @{ is_prefix-path && is_prefix-name }| ) ##NO_TEXT.
    ENDIF.
  ...
  METHOD convert_struc.
    ...
    ls_root-type  = zif_mcp_ajson_types=>node_type-object.
  ...
  METHOD convert_table.
    ...
    ls_root-path  = is_prefix-path.
    ls_root-name  = is_prefix-name.
    ls_root-type  = zif_mcp_ajson_types=>node_type-array.
    ls_root-index = iv_index.
    ls_root-order = iv_item_order.
  ...
  METHOD insert_value_with_type.
    ...
    IF io_type->type_kind CO lif_kind=>texts OR
      ...
      IF iv_type = zif_mcp_ajson_types=>node_type-boolean AND iv_data <> 'true' AND iv_data <> 'false'.
        zcx_mcp_ajson_error=>raise( |Unexpected boolean value [{ iv_data }] @{ lv_prefix }| ) ##NO_TEXT.
      ELSEIF iv_type = zif_mcp_ajson_types=>node_type-null AND iv_data IS NOT INITIAL.
        zcx_mcp_ajson_error=>raise( |Unexpected null value [{ iv_data }] @{ lv_prefix }| ) ##NO_TEXT.
      ELSEIF iv_type = zif_mcp_ajson_types=>node_type-number AND iv_data CN '0123456789. E+-'.
        zcx_mcp_ajson_error=>raise( |Unexpected numeric value [{ iv_data }] @{ lv_prefix }| ) ##NO_TEXT.
      ELSEIF iv_type <> zif_mcp_ajson_types=>node_type-string AND iv_type <> zif_mcp_ajson_types=>node_type-boolean
        AND iv_type <> zif_mcp_ajson_types=>node_type-null AND iv_type <> zif_mcp_ajson_types=>node_type-number.
        zcx_mcp_ajson_error=>raise( |Unexpected type for value [{ iv_type },{ iv_data }] @{ lv_prefix }| ) ##NO_TEXT.
      ENDIF.
    ELSEIF io_type->type_kind CO lif_kind=>numeric.
      IF iv_type <> zif_mcp_ajson_types=>node_type-number.
        zcx_mcp_ajson_error=>raise( |Unexpected value for numeric [{ iv_data }] @{ lv_prefix }| ) ##NO_TEXT.
      ENDIF.
    ELSE.
      zcx_mcp_ajson_error=>raise( |Unexpected type [{ io_type->type_kind }] @{ lv_prefix }| ) ##NO_TEXT.
    ENDIF.
...
INTERFACE lif_mutator_runner.
  METHODS run
    IMPORTING
      it_source_tree TYPE zif_mcp_ajson_types=>ty_nodes_ts
    EXPORTING
      et_dest_tree   TYPE zif_mcp_ajson_types=>ty_nodes_ts
    RAISING
      zcx_mcp_ajson_error.
ENDINTERFACE.
...
CLASS lcl_filter_runner DEFINITION FINAL.
  PUBLIC SECTION.
    ...
    CLASS-METHODS new
      IMPORTING
        ii_filter          TYPE REF TO zif_mcp_ajson_filter
      RETURNING
  ...
  PRIVATE SECTION.
    DATA mi_filter TYPE REF TO zif_mcp_ajson_filter.
    DATA mr_source_tree TYPE REF TO zif_mcp_ajson_types=>ty_nodes_ts.
    DATA mr_dest_tree TYPE REF TO zif_mcp_ajson_types=>ty_nodes_ts.

    METHODS walk
      IMPORTING
        iv_path   TYPE string
      CHANGING
        cs_parent TYPE zif_mcp_ajson_types=>ty_node OPTIONAL
      RAISING
        zcx_mcp_ajson_error.
...
CLASS lcl_filter_runner IMPLEMENTATION.
  ...
  METHOD walk.
    ...
    DATA ls_node TYPE zif_mcp_ajson_types=>ty_node.
    DATA lv_tab_key TYPE string.

    IF cs_parent-type = zif_mcp_ajson_types=>node_type-array.
      lv_tab_key = 'array_index'. " path + index
    ENDIF.

    LOOP AT mr_source_tree->* INTO ls_node USING KEY (lv_tab_key) WHERE path = iv_path.
      CASE ls_node-type.
        WHEN zif_mcp_ajson_types=>node_type-boolean OR zif_mcp_ajson_types=>node_type-null
          OR zif_mcp_ajson_types=>node_type-number OR zif_mcp_ajson_types=>node_type-string.

          IF mi_filter->keep_node( ls_node ) = abap_false.
            CONTINUE.
          ENDIF.

        WHEN zif_mcp_ajson_types=>node_type-array OR zif_mcp_ajson_types=>node_type-object.

          IF mi_filter->keep_node(
              is_node  = ls_node
              iv_visit = zif_mcp_ajson_filter=>visit_type-open ) = abap_false.
            CONTINUE.
          ENDIF.
          ...
          IF mi_filter->keep_node(
              is_node  = ls_node
              iv_visit = zif_mcp_ajson_filter=>visit_type-close ) = abap_false.
            CONTINUE.
          ENDIF.

        WHEN OTHERS.
          zcx_mcp_ajson_error=>raise( |Unexpected node type { ls_node-type }| ) ##NO_TEXT.
      ENDCASE.
...
CLASS lcl_mapper_runner DEFINITION FINAL.
  PUBLIC SECTION.
    ...
    CLASS-METHODS new
      IMPORTING
        ii_mapper          TYPE REF TO zif_mcp_ajson_mapping
      RETURNING
  ...
  PRIVATE SECTION.
    DATA mi_mapper TYPE REF TO zif_mcp_ajson_mapping.
    DATA mr_source_tree TYPE REF TO zif_mcp_ajson_types=>ty_nodes_ts.
    DATA mr_dest_tree TYPE REF TO zif_mcp_ajson_types=>ty_nodes_ts.
    ...
    METHODS process_deep_node
      IMPORTING
        ...
        iv_node_type    TYPE zif_mcp_ajson_types=>ty_node-type
      RAISING
        zcx_mcp_ajson_error.
...
CLASS lcl_mapper_runner IMPLEMENTATION.
  ...
  METHOD lif_mutator_runner~run.
    ...
    IF sy-subrc <> 0
      OR NOT ( <root>-type = zif_mcp_ajson_types=>node_type-array OR <root>-type = zif_mcp_ajson_types=>node_type-object ).
  ...
  METHOD process_deep_node.
    ...
    LOOP AT mr_source_tree->* ASSIGNING <item> WHERE path = iv_path.
      ...
      IF iv_node_type <> zif_mcp_ajson_types=>node_type-array.
      ...
      IF <item>-type = zif_mcp_ajson_types=>node_type-array OR <item>-type = zif_mcp_ajson_types=>node_type-object.
        process_deep_node(
...
CLASS lcl_mutator_queue IMPLEMENTATION.
  ...
  METHOD lif_mutator_runner~run.
    ...
    DATA lr_buf TYPE REF TO zif_mcp_ajson_types=>ty_nodes_ts.
