  ...
  CLASS tlcl_mcp_test_req_complete IMPLEMENTATION.
    ...
    METHOD prompt_json.
      ...
      result->set_string( iv_path = '/ref/type'       iv_val = zcl_mcp_req_complete=>ref_type-prompt ).
      result->set_string( iv_path = '/ref/name'       iv_val = name ).
      result->set_string( iv_path = '/argument/name'  iv_val = arg_name ).
      result->set_string( iv_path = '/argument/value' iv_val = arg_value ).
    ENDMETHOD.

    METHOD resource_json.
      result = zcl_mcp_ajson=>create_empty( ).
      result->set_string( iv_path = '/ref/type'       iv_val = zcl_mcp_req_complete=>ref_type-resource ).
      result->set_string( iv_path = '/ref/uri'        iv_val = uri ).
      result->set_string( iv_path = '/argument/name'  iv_val = arg_name ).
      result->set_string( iv_path = '/argument/value' iv_val = arg_value ).
    ENDMETHOD.
    ...
    METHOD prompt_ref_parsed.
      ...
      cl_abap_unit_assert=>assert_equals( exp = zcl_mcp_req_complete=>ref_type-prompt
                                          ...
                                          msg = 'ref_type' ).
      cl_abap_unit_assert=>assert_equals( exp = 'greet' act = req->get_ref_name( )  msg = 'ref_name' ).
      cl_abap_unit_assert=>assert_equals( exp = ''      act = req->get_ref_uri( )   msg = 'ref_uri empty' ).
      cl_abap_unit_assert=>assert_equals( exp = 'lang'  act = req->get_argument_name( )  msg = 'arg_name' ).
      cl_abap_unit_assert=>assert_equals( exp = 'en'    act = req->get_argument_value( ) msg = 'arg_value' ).
    ENDMETHOD.
    ...
    METHOD argument_value_optional.
      ...
      json->set_string( iv_path = '/ref/type'      iv_val = zcl_mcp_req_complete=>ref_type-prompt ).
      json->set_string( iv_path = '/ref/name'      iv_val = 'p' ).
      json->set_string( iv_path = '/argument/name' iv_val = 'x' ).
      " no /argument/value

      DATA(req) = NEW zcl_mcp_req_complete( json ).
      cl_abap_unit_assert=>assert_equals( exp = '' act = req->get_argument_value( )
                                          msg = 'value defaults to empty' ).
    ENDMETHOD.

    METHOD context_parsed.
      DATA(json) = prompt_json( ).
      json->set_string( iv_path = '/context/arguments/region' iv_val = 'eu' ).
      json->set_string( iv_path = '/context/arguments/stage'  iv_val = 'prod' ).

      DATA(req) = NEW zcl_mcp_req_complete( json ).
      cl_abap_unit_assert=>assert_true( act = req->has_context( ) msg = 'has_context' ).
      cl_abap_unit_assert=>assert_bound( act = req->get_context_json( ) msg = 'context_json bound' ).
    ...
    METHOD no_context.
      ...
      cl_abap_unit_assert=>assert_false( act = req->has_context( ) msg = 'has_context false' ).
      cl_abap_unit_assert=>assert_not_bound( act = req->get_context_json( ) msg = 'context_json unbound' ).
    ENDMETHOD.

    METHOD meta_parsed.
      DATA(json) = prompt_json( ).
      json->set_string( iv_path = '/_meta/token' iv_val = 'abc123' ).

      DATA(req) = NEW zcl_mcp_req_complete( json ).
      cl_abap_unit_assert=>assert_bound( act = req->get_meta( ) msg = 'meta bound' ).
    ...
    METHOD no_meta.
      DATA(req) = NEW zcl_mcp_req_complete( prompt_json( ) ).
      cl_abap_unit_assert=>assert_not_bound( act = req->get_meta( ) msg = 'meta unbound' ).
    ENDMETHOD.
    ...
    METHOD unknown_ref_type.
      DATA(json) = zcl_mcp_ajson=>create_empty( ).
      json->set_string( iv_path = '/ref/type'      iv_val = 'ref/unknown' ).
      json->set_string( iv_path = '/argument/name' iv_val = 'x' ).
      TRY.
          NEW zcl_mcp_req_complete( json ).
    ...
    METHOD missing_ref_uri.
      DATA(json) = zcl_mcp_ajson=>create_empty( ).
      json->set_string( iv_path = '/ref/type'      iv_val = zcl_mcp_req_complete=>ref_type-resource ).
      json->set_string( iv_path = '/argument/name' iv_val = 'x' ).
      " no /ref/uri
      TRY.
          NEW zcl_mcp_req_complete( json ).
