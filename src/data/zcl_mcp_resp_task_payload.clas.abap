CLASS zcl_mcp_resp_task_payload DEFINITION
  ...
  PRIVATE SECTION.
    ...
    TYPES: BEGIN OF text_item,
             ...
             text TYPE string,
           END OF text_item.
    TYPES text_items TYPE STANDARD TABLE OF text_item WITH DEFAULT KEY.
...
CLASS zcl_mcp_resp_task_payload IMPLEMENTATION.
  METHOD zif_mcp_internal~generate_json.
    DATA item LIKE LINE OF int_content.
  ...
  METHOD add_text_content.
    DATA temp1 TYPE text_item.
    CLEAR temp1.
    temp1-type = 'text'.
    temp1-text = text.
    APPEND temp1 TO int_content.
  ENDMETHOD.
  ...
  METHOD set_related_task.
    ...
    IF int_meta IS NOT BOUND.
      IF int_prebuilt IS BOUND AND int_prebuilt->exists( '/_meta' ) IS NOT INITIAL.
