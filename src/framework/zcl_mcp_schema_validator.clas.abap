  ...
  PRIVATE SECTION.
    TYPES:
      ...
      BEGIN OF error,
        ...
        message TYPE string,
      END OF error,
      errors TYPE STANDARD TABLE OF error WITH DEFAULT KEY.
...
CLASS zcl_mcp_schema_validator IMPLEMENTATION.
  ...
  METHOD validate.
        DATA root_type TYPE string.
          DATA msg TYPE string.
    FIELD-SYMBOLS <error> LIKE LINE OF detailed_errors.
      DATA temp1 LIKE LINE OF error_list.
    ...
    TRY.
        ...
        root_type = schema->get_string( '/type' ).
        IF root_type <> 'object'.
          " Schema root must be of type "object", but found &1
          
          MESSAGE e100(zmcp) WITH root_type INTO msg.
          add_error( path    = '/'
                     message = msg ).
          result = abap_false.
          RETURN.
    ...
    LOOP AT detailed_errors ASSIGNING <error>.
      
      temp1 = |{ <error>-path }: { <error>-message }|.
      APPEND temp1 TO error_list.
    ENDLOOP.
  ENDMETHOD.
  ...
  METHOD validate_object.
    ...
    TRY.
        ...
        IF schema->exists( |{ schema_path }required| ) IS NOT INITIAL.
          ...
          LOOP AT required_props ASSIGNING <prop>.
            IF NOT property_exists( json_path = |{ json_path }{ <prop> }|
                                    json      = json ) IS NOT INITIAL.
              " Required property "&1" is missing
              
              MESSAGE e102(zmcp) WITH <prop> INTO msg.
    ...
    LOOP AT props_names ASSIGNING <prop_name>.
      ...
      IF property_exists( json_path = prop_json_path
        ...
        prop_valid = validate_property( json_path   = prop_json_path
  ...
  METHOD validate_array.
    ...
    DATA schema_items_path TYPE string.
    DATA items_type TYPE string.
        DATA msg TYPE string.
    DATA item_indices TYPE string_table.
        DATA array_path LIKE json_path.
        DATA path_length TYPE i.
          DATA offset TYPE i.
          DATA last_char TYPE string.
        DATA item_count TYPE i.
        FIELD-SYMBOLS <idx> LIKE LINE OF item_indices.
      DATA min_items TYPE i.
      DATA max_items TYPE i.
      DATA item_path TYPE string.
          DATA item_valid TYPE abap_bool.
              DATA item_type TYPE zif_mcp_ajson_types=>ty_node_type.
    schema_items_path = |{ schema_path }items|.
    ...
    IF schema->exists( |{ schema_path }minItems| ) IS NOT INITIAL.
      
      min_items = schema->get_integer( |{ schema_path }minItems| ).
    ...
    IF schema->exists( |{ schema_path }maxItems| ) IS NOT INITIAL.
      
      max_items = schema->get_integer( |{ schema_path }maxItems| ).
    ...
    LOOP AT item_indices ASSIGNING <idx>.
      ...
      item_path = |{ array_path }/{ <idx> }|.

      " Check item type and validate accordingly
      CASE items_type.
        WHEN 'object'.
          
          item_valid = validate_object( json_path   = |{ item_path }/|
        ...
        WHEN 'boolean'.
          ...
          TRY.
              ...
              item_type = json->get_node_type( item_path ).
              IF item_type <> 'bool'.
  ...
  METHOD validate_string.
        DATA node_type TYPE zif_mcp_ajson_types=>ty_node_type.
          DATA msg TYPE string.
    DATA is_valid TYPE abap_bool VALUE abap_true.
    DATA string_value TYPE string.
    DATA strlength TYPE i.
      DATA min_length TYPE i.
      DATA max_length TYPE i.
      DATA enum_values TYPE string_table.
      DATA temp2 LIKE sy-subrc.
        DATA enum_list TYPE string.
        FIELD-SYMBOLS <enum> LIKE LINE OF enum_values.
    TRY.
        
        node_type = json->get_node_type( json_path ).
        IF node_type <> 'str'.
          " Expected type "string" but got "&1"
          
          MESSAGE e119(zmcp) WITH node_type INTO msg.
          add_error( path    = json_path
                     message = msg ).
          result = abap_false.
          RETURN.
    ...
    string_value = json->get_string( json_path ).

    " Check minLength - only if it exists in schema
    
    IF schema->exists( |{ schema_path }/minLength| ) IS NOT INITIAL.
      
      min_length = schema->get_integer( |{ schema_path }/minLength| ).
    ...
    IF schema->exists( |{ schema_path }/maxLength| ) IS NOT INITIAL.
      
      max_length = schema->get_integer( |{ schema_path }/maxLength| ).
      strlength = strlen( string_value ).
    ...
    IF schema->exists( |{ schema_path }/enum| ) IS NOT INITIAL.
      
      enum_values = schema->array_to_string_table( |{ schema_path }/enum| ).

      " Check if value is in enum
      
      READ TABLE enum_values WITH KEY table_line = string_value TRANSPORTING NO FIELDS.
      temp2 = sy-subrc.
      IF string_value IS NOT INITIAL AND NOT temp2 = 0.
        " Build readable enum list for error message
        
        
        LOOP AT enum_values ASSIGNING <enum>.
  ...
  METHOD validate_number.
    ...
    TRY.
        ...
        IF node_type <> 'num'.
          ...
          MESSAGE e123(zmcp) WITH node_type INTO msg.
          add_error( path    = json_path
                     message = msg ).
          result = abap_false.
          RETURN.
    ...
    number_value = json->get_number( json_path ).

    " Check minimum - only if it exists in schema
    IF schema->exists( |{ schema_path }/minimum| ) IS NOT INITIAL.
      
      minimum = schema->get_number( |{ schema_path }/minimum| ).
    ...
    IF schema->exists( |{ schema_path }/maximum| ) IS NOT INITIAL.
      
      maximum = schema->get_number( |{ schema_path }/maximum| ).
  ...
  METHOD validate_integer.
    ...
    TRY.
        ...
        IF node_type <> 'num'.
          ...
          MESSAGE e126(zmcp) WITH node_type INTO msg.
          add_error( path    = json_path
                     message = msg ).
          result = abap_false.
          RETURN.
    ...
    integer_value = json->get_integer( json_path ).

    " Check minimum - only if it exists in schema
    IF schema->exists( |{ schema_path }/minimum| ) IS NOT INITIAL.
      
      minimum = schema->get_integer( |{ schema_path }/minimum| ).
    ...
    IF schema->exists( |{ schema_path }/maximum| ) IS NOT INITIAL.
      ...
      maximum = schema->get_integer( |{ schema_path }/maximum| ).
  ...
  METHOD add_error.
    ...
    DATA temp3 TYPE zcl_mcp_schema_validator=>error.
    CLEAR temp3.
    temp3-path = path.
    temp3-message = message.
    APPEND temp3 TO detailed_errors.
  ENDMETHOD.
ENDCLASS.
