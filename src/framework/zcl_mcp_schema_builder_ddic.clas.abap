...
CLASS zcl_mcp_schema_builder_ddic IMPLEMENTATION.
  METHOD constructor.
    DATA override LIKE LINE OF field_overrides.
    me->structure_name  = structure_name.
    me->field_overrides = field_overrides.

    " Validate overrides
    
    LOOP AT field_overrides INTO override.
    ...
    CREATE OBJECT builder TYPE zcl_mcp_schema_builder.
  ...
  METHOD process_structure.
    ...
    DATA field_info LIKE LINE OF field_list.
      DATA field_name_lower TYPE string.
      DATA temp1 TYPE string.
      DATA current_field_path LIKE temp1.
    ...
    LOOP AT field_list INTO field_info.
      " For field path, we need just the field name when at root level
      
      field_name_lower = to_lower( field_info-fieldname ).
      
      IF field_path IS INITIAL.
        temp1 = field_name_lower.
      ELSE.
        temp1 = build_field_path( base_path = field_path field_name = field_name_lower ).
      ENDIF.
      
      current_field_path = temp1.
  ...
  METHOD process_field.
    ...
    DATA nested_field_path LIKE temp2.
    DATA field_name TYPE string.
    DATA description TYPE string.
    DATA required TYPE abap_bool.
        DATA nested_builder TYPE REF TO zcl_mcp_schema_builder.
        DATA struct_name TYPE string.
        DATA array_builder TYPE REF TO zcl_mcp_schema_builder.
        DATA json_type TYPE string.
            ...
            DATA temp1 TYPE i.
    IF field_path IS INITIAL.
      temp2 = to_lower( field_info-fieldname ).
    ELSE.
      temp2 = field_path.
    ENDIF.
    
    nested_field_path = temp2.

    
    field_name = get_field_name( field_info = field_info
                                       field_path = nested_field_path ).
    
    description = get_field_description( field_info = field_info
                                               field_path = nested_field_path ).
    
    required = is_field_required( field_info = field_info
                                         field_path = nested_field_path ).
    ...
    CASE field_info-datatype.
      WHEN 'STRU'.
        ...
        nested_builder = current_builder->begin_object( name        = field_name
      ...
      WHEN 'TTYP'.
        ...
        array_builder = current_builder->begin_array( name        = field_name
                                                            description = description
                                                            required    = required ).
      ...
      WHEN OTHERS.
        ...
        json_type = map_abap_type( field_info ).

        CASE json_type.
          WHEN 'string'.
            
            enum_values = get_domain_values( field_info-domname ).

            " Get field length
            
            max_length = get_field_length( field_info ).
  ...
  METHOD get_field_name.
    ...
    DATA override TYPE zcl_mcp_schema_builder_ddic=>def_field_override.
    override = get_field_override( field_path ).
  ...
  METHOD get_domain_values.
    DATA domain_values TYPE TABLE OF dd07v.
      DATA domain_value LIKE LINE OF domain_values.
    CLEAR result.
    ...
    IF sy-subrc = 0.
      ...
      LOOP AT domain_values INTO domain_value.
  ...
  METHOD get_field_override.
    DATA last_dot TYPE i.
      DATA field_name_only TYPE string.
    CLEAR result.
    ...
    last_dot = find( val = field_path
                           sub = '.'
                           occ = -1 ).
    IF last_dot >= 0.
      
      field_name_only = substring( val = field_path
                                         off = last_dot + 1 ).
